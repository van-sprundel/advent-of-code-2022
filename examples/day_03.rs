...
fn part_1() -> Result<i32> {
    ...
    INPUT.lines().for_each(|line: &str| {
        ...
        let mut temp_res = right.iter().filter(|c| left.contains(c)).collect::<Vec<_>>();
        temp_res.dedup();
        res.extend(temp_res);
    });
...
fn part_2() -> Result<i32> {
    let mut res: Vec<char> = vec![];
    INPUT.lines().collect::<Vec<_>>().chunks(3).for_each(|lines| {
        let line1 = lines[0];
        let line2 = lines[1];
        let line3 = lines[2];

        let res1 = line1.chars().filter(|c| line2.contains(*c));
        let res2 = line2.chars().filter(|c| line3.contains(*c)).collect::<Vec<_>>();
